...
const showMovies = async (data, genreId, rowIndex) => {
  ...
  slideLeft.innerHTML = `<span class='fa fa-angle-left' id='${rowIndex}'></span>`;

  const slideRight = document.createElement("div");
  slideRight.classList.add("slide", "right");
  slideRight.innerHTML = `<span class='fa fa-angle-right' id='${rowIndex}'></span>`;

  newRow.appendChild(movieContainer);
  newRow.appendChild(loadButton);
  newRow.appendChild(slideLeft);
  newRow.appendChild(slideRight);
